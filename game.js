...
function placeholderBackground(ctx, width, height, time) { ctx.save(); ctx.fillStyle = '#182b52'; ctx.fillRect(0, 0, width, height); ctx.restore(); }
function placeholderGround(ctx, width, height, groundHeight, offset) { ctx.save(); ctx.fillStyle = '#263238'; ctx.fillRect(0, height - groundHeight, width, groundHeight); ctx.restore(); }
function placeholderBird(ctx, x, y, size, velocity) { ctx.save(); ctx.fillStyle = '#ffd166'; ctx.fillRect(x - size / 2, y - size / 2, size, size); ctx.restore(); }
function placeholderPipe(ctx, x, gapTop, gapBottom, pipeWidth, height) { ctx.save(); ctx.fillStyle = '#54c27a'; ctx.fillRect(x, 0, pipeWidth, gapTop); ctx.fillRect(x, gapBottom, pipeWidth, height - gapBottom); ctx.restore(); }
...
let state = 'ready', velocity = 0, y = CONFIG.canvasHeight / 2;
let pipes = [], pipesMade = 0, groundOffset = 0, lastGapTop = CONFIG.canvasHeight / 2 - CONFIG.pipeGap / 2;
let score = 0, checkpoint = 0, secondsSinceCrash = 0, currentMode = 'normal', bestScore = 0;
...
function showReady() { overlayTitle.textContent = CONFIG.title; overlayText.textContent = ['Press Space, click or tap to start.', soundLine()].join('\n'); overlay.hidden = false; }
function showGameOver() {
  ...
  lines.push(soundLine()); overlayText.textContent = lines.join('\n'); overlay.hidden = false;
}
function press() {
...
function crash() {
  ...
  if (score > bestScore) { bestScore = score; try { localStorage.setItem('cloneWarsBest', String(bestScore)); } catch (error) {} }
  checkpoint = CONFIG.fix === 'checkpoints' ? Math.floor(score / 10) * 10 : 0;
  sr.textContent = 'Game over. Score ' + score + '. Best ' + bestScore + '.'; showGameOver();
}
function frame(now) {
  const seconds = Math.min(0.05, Math.max(0, (now - (frame.last || now)) / 1000)); frame.last = now;
  ...
  if (state === 'playing') {
    velocity += CONFIG.gravity * seconds; y += velocity * seconds; groundOffset += pipeSpeed * seconds;
  ...
  if (state === 'playing') { ctx.save(); ctx.font = 'bold 42px system-ui, sans-serif'; ctx.textAlign = 'center'; ctx.lineWidth = 6; ctx.strokeStyle = '#172033'; ctx.strokeText(String(score), CONFIG.canvasWidth / 2, 64); ctx.fillStyle = '#fff'; ctx.fillText(String(score), CONFIG.canvasWidth / 2, 64); ctx.restore(); }
